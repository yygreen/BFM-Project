...
- Real pricing from the client's own sheet (Sep 2026), superseding the Aug 2026 store-widget audit: Qatar 1.5, Aeroplan / Flying Blue / American / Avianca / BA / Asia Miles / EVA / KrisFlyer / Turkish 1.7, Emirates / Etihad / United 1.9, Atmos (Alaska) 2. Delivery is now stated in **business days** ("Within 3 business days"; EVA and United "Within 7"); Delta keeps its older audited 1.95 / 24h until the client re-confirms it. All of these have `priceVerified: true` / `deliveryVerified: true`. `src/lib/delivery.ts → deliveryHours()` makes hour and business-day windows sortable together — use it for any "fastest window" logic.
- **`priceVerified` / `deliveryVerified` replace the old `// PLACEHOLDER` comments.** Anything `false` (ANA, Frontier, JetBlue, Lufthansa, Southwest, Iberia, Qantas) needs a **real, confirmed value before launch** — then flip the flag to `true`.
